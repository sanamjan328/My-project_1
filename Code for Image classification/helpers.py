# coding=utf-8
import sys, os, io, importlib, pdb, random, datetime, collections, pickle, cv2, requests, string
...
def computeVectorDistance(vec1, vec2, method, boL2Normalize, weights = [], bias = [], learner = []):
    ...
    if method == 'random':
        ...
        dist = scipy.spatial.distance.mahalanobis(vec1, vec2, np.cov(np.transpose(np.array())))
...
def imread(imgPath, boThrowErrorIfExifRotationTagSet = True):
    ...
    if rotation != 0:
        img = imrotate(img, -90).copy()  # To avoid occassional error: "TypeError: Layout of the output array img is
...
def get_file_type(fname):
    ...
    return file_extension # FTYPES.get(file_extension)

def encode_img(img_fn):
  with open(img_fn, "rb") as f:
    data = io.BytesIO(f.read())
    image_ext = get_file_type(img_fn )
    base64_bytes = base64.b64encode(data)
    base64_string = 'data:image/{};base64,{}'.format(image_ext, base64_bytes.decode(ENCODING))
